...
export default function ProblemTypes() {
  ...
  const handleSave = async () => {
    ...
    const url = editingItem ? `/api/problem-types/${editingItem.id}` : '/api/problem-types';
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await fetch(`/api/problem-types/${id}`, { method: 'DELETE' });
      fetchTypes();
    } catch (err) {
      console.error(err);
    }
  ...
  return (
    <div>
      <div className="mb-6 flex justify-between items-center">
        <h2 className="text-2xl font-bold text-gray-800">{t('problemTypes')}</h2>
        <button
          onClick={() => openModal()}
              ...
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-lg leading-6 font-medium text-gray-900">
                    {editingItem ? t('edit') : t('add')} {t('problemTypes')}
                  </h3>
